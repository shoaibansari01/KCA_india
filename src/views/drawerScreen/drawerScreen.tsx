        ...
        <SafeAreaView>
            <View style={style.drawerContainerModal}>
                <View style={{ padding: 20 }}>
                    ...
                    <View style={{ marginTop: 30, flexDirection: "row", marginHorizontal: 20, alignItems: "center" }}>
                        <View style={style.profileCircleIcon}>
                            <Icon name="person-outline" color="#fff" size={20} />
                        </View>
                        <View style={{ flexDirection: "column" }}>
                            <View>
                                <Text style={[style.fs22, style.boldText, style.textColorBlack]}>{authData?.user?.name?.substring(0, 10) ?? ""}</Text>
                            </View>
                    ...
                    <ScrollView>
                        <View>
                            {[{ icon: "favorite-outline", name: "My Favorites", redirect: "Favourite" }, { icon: "dashboard", name: "My Dashboard", redirect: "MyDasboards" }, { icon: "contact-page", name: "Certificate", redirect: "Certificates" }, { icon: "payment", name: "Payment Details", redirect: "PaymentDetails" },
                            // { icon: "feedback", name: "Feedback", redirect: "" }, 
                            // { icon: "ads-click", name: "Ads", redirect: "" },
                            { icon: "info", name: "About KCA", redirect: "AboutUs" },
                            { icon: "assignment", name: "Term & Condition", redirect: "TermAndCond" },
                            { icon: "policy", name: "Policys", redirect: "Policys" },
                            // { icon: "support", name: "Support", redirect: "" },
                            // { icon: "settings", name: "Setting", redirect: "" },
                            { icon: "logout", name: "Logout", redirect: "", callBack: handleLogout }].map(({ icon, name, redirect, callBack }: any, i) =>
                                <TouchableOpacity key={i} onPress={() => { (name != "Logout") && navigation.navigate(redirect), callBack && callBack() }}>
                                    <ListItem key={i} bottomDivider containerStyle={{ backgroundColor: 'transparent', paddingVertical: 10, paddingHorizontal: 0 }}>
                                        <View style={style.circleIcon}><Icon size={18} color="#fff" name={icon} /></View>
                                        <ListItem.Content >
                                            <ListItem.Title style={style.userOptDtls.text}>{name}</ListItem.Title>
                                        </ListItem.Content>
...
export const HorizontalLine = () => <View style={{ height: 1, backgroundColor: 'black', marginBottom: 20, marginTop: 15 }} />
