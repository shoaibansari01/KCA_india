...
import {RAZORPAY_KEY, postReq, FormApi} from '../../helper/http';
...
const FinalFeesConformation = ({route, navigation}: any) => {
  ...
  const getStudentsInfo: any = ({clsName, noOfStd}: any) => {
    const fee =
      national?.classData?.length &&
      national?.classData?.find((e: any) => e?.cls == clsName)?.fees;
  ...
  const totalFees = () =>
    national?.finalData &&
    Object.keys(national?.finalData)?.reduce((a, c) => {
  ...
  const studentData =
    national?.finalData &&
    Object.keys(national?.finalData)?.reduce((a: any, c: any) => {
      a.push({
        className: c,
        no_of_student: national?.finalData[c],
        fees: getStudentsInfo({clsName: c, noOfStd: national?.finalData[c]})
          ?.fee,
        total: getStudentsInfo({clsName: c, noOfStd: national?.finalData[c]})
          ?.total,
      });
      return a;
    }, []);
  ...
  const payment = async () => {
    ...
    const _ = await postReq({
      url: 'create-order',
      data: {
        amount: (totalFees() ?? artData?.value) * 100,
        currency: dollar && level == 'global' ? 'USD' : 'INR',
        isNational: level != 'global',
        isGlobal: level == 'global',
        userId: authData?.user?.userId,
        form_id: global?.values?._id,
        noOfArtWork: artData?.numberOfArtWork,
        studentData,
      },
    });
    if (_?.data?.orderRes?.id) {
      ...
      if (res?.razorpay_order_id) {
        const status = await postReq({
          url: 'payment-status',
          data: {
            id: _?.data?.payment?._id,
            razorpay_order_id: res?.razorpay_order_id,
            razorpay_payment_id: res?.razorpay_payment_id,
            razorpay_signature: res?.razorpay_signature,
            razorpay_details: res,
          },
        });
    ...
    Animated.timing(fadeAnim, {
      ...
      redirect &&
        navigation.navigate('UserDashboard', {
          data: {values: {...global?.values}, level},
        });
    });
  };
  ...
  const uploadPaymentReceipt = async () => {
    console.log('Upload payment receipt clicked');
    ...
    console.log('File selected:', fileResponse[0]?.name);
    setFileErr('');
    setIsLoading(true);
    ...
    const safeTotalFees = totalFees() || artData?.value || 0;
    const safeSymbol = global?.symbol || national?.symbol || '₹';

    formData.append(
      'data',
      JSON.stringify({
        userId: authData?.user?.userId,
        registrationFormId: global?.values?._id,
        classData: safeStudentData,
        totalFee: `${safeSymbol} ${
          safeTotalFees.toFixed ? safeTotalFees.toFixed(2) : safeTotalFees
        }`,
      }),
    );

    console.log('Debug data before sending:', {
      userId: authData?.user?.userId,
      registrationFormId: global?.values?._id,
      nationalFinalData: national?.finalData,
      studentData: studentData,
      totalFees: totalFees(),
      nationalData: national,
      fileName: fileResponse[0]?.name,
    });

    console.log('Uploading payment receipt...');

    try {
      console.log('Calling postReq with formData...');
      
      // Direct API call as fallback
      console.log('Making direct FormApi call...');
      const directResponse = await FormApi.post('upload-school-payment-receipt', formData);
      console.log('Direct FormApi response:', directResponse);
      
      const uploadResult = directResponse?.data;
      
      console.log('Upload result:', uploadResult);

      // Check for successful upload - should have data and message
      if (uploadResult && uploadResult.data && uploadResult.message) {
        console.log('Upload successful - Receipt saved with ID:', uploadResult.data._id);
        Alert.alert('Success', uploadResult.message);
        handlePaymentModal(true, false);
      } else if (uploadResult && uploadResult.statusCode === 500) {
        console.log('Server error:', uploadResult);
        Alert.alert('Server Error', uploadResult.message || 'Internal server error occurred.');
      } else {
        console.log('Upload failed - unexpected response:', uploadResult);
        Alert.alert(
          'Upload Failed', 
      ...
      const errorMessage = error?.message || 'Failed to upload payment receipt. Please try again.';
            ...
            <View style={style.finalFeeConfCard}>
              <View>
                <View
                  ...
                  <Text style={style.confirmEntryCardDetails}>
                    Total {level != 'global' ? 'Participant' : 'Art Work'}
                  </Text>
              ...
              <View>
                <View
                  ...
                  <Text style={style.totalFinalFees}>
                    {global?.symbol} {totalFees()?.toFixed(2) ?? artData?.value}
                  </Text>
                </View>
              </View>
              <View
                ...
                <View
                  ...
                  <Button
                    ...
                    title={isLoading ? 'Uploading...' : 'Submit Receipt'}
